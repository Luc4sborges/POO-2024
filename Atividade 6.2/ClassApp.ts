...
class App {
    ...
    consultarConta(): void {
        ...
        if (conta) {
            console.log(`Número: ${conta.getNumero()}, Saldo: ${conta.getSaldo()}`);
        }
    }
    ...
    consultarCliente(): void {
        ...
        if (cliente) {
            console.log(`CPF: ${cliente.getCpfCliente()}, Contas: ${cliente.getContasCliente().map(conta => conta.getNumero()).join(', ')}`);
        }
    }
    ...
    listarContasCliente(): void {
        ...
        if (contas.length > 0) {
            contas.forEach(conta => console.log(`Número: ${conta.getNumero()}, Saldo: ${conta.getSaldo()}`));
        }
    }
    ...
    listarContasSemCliente(): void {
        ...
        for (let i = 0; i < this.banco.quantidadeDeContas(); i++) {
            let contaTemCliente = false;
            for (let j = 0; j < this.banco.clientes.length; j++) {
                for (let k = 0; k < this.banco.clientes[j].getContasCliente().length; k++) {
                    if (this.banco.clientes[j].getContasCliente()[k] === this.banco.consultarConta(this.banco.consultarConta().getNumero())) {
            ...
            if (!contaTemCliente) {
                contasSemCliente.push(this.banco.consultarConta(this.banco.consultarConta().getNumero()));
            }
        }
        ...
        for (let i = 0; i < contasSemCliente.length; i++) {
            console.log(`Número: ${contasSemCliente[i].getNumero()}, Saldo: ${contasSemCliente[i].getSaldo()}`);
        }
        ...
        for (let i = 0; i < contasSemCliente.length; i++) {
            if (contasSemCliente[i].getNumero() === numeroConta) {
